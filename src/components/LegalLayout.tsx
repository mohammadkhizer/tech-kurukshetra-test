...
export function LegalLayout({ title, subtitle, lastUpdated, sections, children }: LegalLayoutProps) {
  return (
    <div className="min-h-screen bg-tk-bg text-tk-text">
      {/* Header */}
      <section className="pt-20 pb-12 px-4 sm:px-6 border-b border-tk-border">
        <div className="max-w-5xl mx-auto">
          ...
          >
            <div className="text-xs text-tk-accent tracking-[0.3em] uppercase">◈ LEGAL</div>
            <h1 className="text-4xl sm:text-5xl font-black tracking-tighter font-headline leading-[0.9] text-tk-text">{title}</h1>
            <p className="text-tk-text-muted text-sm">{subtitle}</p>
            <div className="text-[10px] text-tk-text-muted/60 uppercase tracking-[0.2em] mt-1">Last updated: {lastUpdated}</div>
          </motion.div>
        </div>
      </section>
